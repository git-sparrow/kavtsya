...
Consequence: the auth and account model must support a "roles" concept — an account can be Customer-only or Customer+CafeOwner. CafeOwner Mode is unlocked when the user registers a Café — a **post-signup action from Settings**, not a branch in the signup flow (the code never had a combined-signup step; wording corrected 2026-07-12). Once unlocked, CafeOwner Mode becomes the account's **primary, default surface** and the Customer surface a secondary Settings excursion — see ADR 0015.
