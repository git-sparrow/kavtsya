...
| **Maestro** (checked 2026-09-04) | `https://docs.maestro.dev/llms-full.txt` | **full content** (~400 KB) | The gallery/E2E driver (`apps/mobile/.maestro/`). One fetch, then `grep` it — cheaper than paging the index. Settled the selector questions the flows depend on: `id:` is regex-based and **unanchored**, `back` is **Android/Web only** (a silent no-op on iOS), and `optional: true` + `label:` work on most commands. Per-command pages also exist as `…/reference/commands-available/<cmd>.md`. `docs.maestro.dev/api-reference/*` 404s. |
