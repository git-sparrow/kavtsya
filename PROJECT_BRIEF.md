...
_Last updated: 2026-07-30 · Status: **building** — scope and architecture locked. **This line + the Roadmap below are the single status home** (AGENTS.md links here; nothing else restates status). Everything spec-frozen on 2026-07-10 is merged: core loop (earn #20, Redemption #22, Ворожка #23), member code (#21), «Зміна» scanner grants + Barista Roster (#80, #97–#99), Pro gating + push campaigns (#24), analytics + free teaser (#25), account deletion + Café archival (#81, with its 6b/7d exit UI #143). Next: provision production (#65) → pilot & store checklist (#58). In parallel: **UI redesign** from the Mari handoff (epic #131, turns #132–#138 and #143 merged — the epic is complete; theme switch #162 and edge-to-edge scroll #165 landed as add-on turns 10–11; client export #139 and café transfer #160 postponed out of v1)._
...
- **Birthday reward — «З Днем народження»** (#180) — optional birthday (day+month only, no year) in Customer settings; a scheduled push + a birthday Ворожка moment on the day. Platform-level delight is Free; a per-Café birthday bonus would be a Pro campaign type later (per-Café Зернятка, ADR 0001, mean a bonus must name a Café). _Filed 2026-08-07._
...
- **Self-check-in via NFC — «Зернятко по дотику»** (#179) — the NFC twin of #61: a passive NTAG 424 DNA tag in a branded puck at the counter; the Customer taps their phone to claim the Зернятко. Same trust model and gates as #61, but the hardware drops from a powered display to a ~$3–5 giveaway, and the tag's SUN mechanism emits a rotating single-use cryptographic token per tap — the ADR 0006 guarantee from the chip itself. _Filed 2026-08-07 from the counter-experience brainstorm; research in `docs/explorations/nfc-tap-to-collect.md`. Whichever of #61/#179 runs first owns the shared rate-limit design (1/Customer/Café/Kyiv business day, #112)._
- **Single-use printed codes — Зернятко for takeaway/delivery** (#181) — batch-generated bearer codes on cup sleeves and delivery bags, redeemed in-app; the only earn path that works with zero café presence (Glovo/Bolt Food). Weakest trust of the self-serve trio (#61/#179/#181) — single-use + batch expiry/revocation + the shared rate limit; earns only, never redeems. _Filed 2026-08-07; open: Free vs Pro under ADR 0011, and whether pilot cafés have real delivery volume._
...
- CafeOwner: **Café logo** (#174) — a Кавовар uploads their café's logo; it leads each row in «МОЇ КАВ'ЯРНІ» on Customer home and sits in the «РЕЖИМ КАВОВАРА» role header, so a Customer who buys at three Cafés stops reading three rows of identical text. **Free on every Plan** — it is Customer-facing (ADR 0011), not a growth tool. _Filed 2026-08-05, not scheduled. Decisions locked in the issue: bytes in Postgres behind an `ImageStore` seam (needs its own ADR; swaps to R2/S3 without touching callers, same pattern as ADR 0007/0012), client-side square crop + server-side re-encode to a canonical 512×512 PNG (strips EXIF, neutralises hostile payloads), a public content-hash read URL as a narrow deliberate exception for non-PII branding that RN's `Image` can't fetch with a session cookie, and the existing monogram `Avatar` as the fallback every Café starts on. Carries the API's first binary upload, first image serving, and first user-generated content shown to other users — the moderation posture is live-on-upload plus an audit trail and a Platform takedown. Undesigned: the Mari handoff has no logo slot anywhere, and the Customer-facing «Поскаржитися» has no home (there is no Customer Café detail screen)._
- CafeOwner: **marketing materials — table-tent/poster PDF generator + printed kit** (#183) — a print-ready PDF (table tent + poster: Café name, join QR, Ворожка teaser) generated from the CafeOwner surfaces, **Free** — every table tent advertises Кавця, so gating our own acquisition surface behind Pro would be self-defeating (deliberate ADR 0011 reading); the professionally **printed-and-shipped kit** is the founding-pilot + Pro perk. The QR hits a server-side redirect under our domain (#1) so already-printed tents upgrade to the App Clip target (gtm item 1) with zero reprints. _Filed 2026-08-07; ops questions (print partner, legal footer) belong to the #58 pilot checklist._
...
5. **Build the vertical slices.** ← in progress. Merged: walking skeleton (#15), auth (#16), Cafés (#17), loyalty config (#18), rotating QR token (#19), core scan + Зернятко ledger (#20), scan-rejection taxonomy (#50), balance fold (#52), Redemption (#22), Ворожка (#23), member code (#21), «Зміна» scanner grants + Barista Roster (#80, #97–#99), role Modes (#96), design tokens (#94), Pro gating + push campaigns (#24), analytics + free teaser (#25), account deletion + Café archival (#81); dev tooling: CI (#64), Argent (#79), dependency-freshness (#105). **The full v1 feature set is live.** Remaining before store/pilot: provision production (#65) → readiness checklist (#58) → branding pass (#67). Architecture/quality frontier (agent fodder, any order): #53, #54, #112–#118 (#111 counter-access consolidation — one `authorizeCounter`, Shift-owned grant predicate — landed; #116 Ворожка pool observability — today's pool count on `/health` — landed; #51 protected-route guard — one default-deny `requireUser` at the app seam, handlers typed against a non-null user — landed; #190 mobile read-path consolidation — the four remaining hand-rolled loaders moved onto `useApiResource`, which now resets on a resource change and warns on an unstable `load`, plus one `useScreenAction` folding read and action errors into a screen's single error line; ADR 0016 records why the app hand-rolls this rather than adopting a query library — landed).
6. **UI redesign** (Mari handoff, epic #131). Full spec + 79 screens vendored at `docs/design/handoff/`; map at `docs/design/redesign-reconciliation.md`. Sliced per turn: tokens #132 → home/redemption/Ворожка #133 → owner scan #134 → owner home/config #135 → settings/analytics #136 → role subscreens #137 → confirm dialogs #138 → delete-account exit #143 (6b + 7d, UI on top of #81, which stayed archival-only). All merged — turns 0–6 (#132–#138) and #143; **the epic is complete.** #143 shipped with two documented deviations from the mock that reverse when #160 lands: screen 7a (the transfer-first blocked dialog) is not built, and 7d's safe action reads «Скасувати» instead of «Краще передати кав'ярню» — a button must not name a destination that doesn't exist. **Café ownership transfer (turns 7b/7c + 8) was split out on 2026-07-27** — out of the epic and out of v1 into #160 (Backlog above), which settles the epic's ADR-0014 conflict archival-first: deletion is never blocked, so #143 builds no blocked dialog. **Turn 9, client-base export (#139), was postponed on 2026-07-27** — out of the epic and out of v1, since it restyles nothing and needs a net-new backend incl. the API's first email infrastructure; it now sits in Backlog above. **Turn 10, the Settings theme switch (#162), landed 2026-07-28** as an add-on filed after the slicing — its design package is vendored at `docs/design/handoff/updates/2026-07-21-theme-toggle/`. It replaced the per-screen `useColorScheme()` reads with one stored preference («Системна» / «Світла» / «Темна») behind a single root provider; the always-dark Ворожка reveal keeps its deliberate override. **Turn 11, edge-to-edge scroll (#165), landed 2026-07-30** — also an add-on, and net-new design rather than handoff (Mari's spec is silent on edge treatment): `Screen` and the Ворожка reveal drop the opaque safe-area band for insets applied as scroll-content padding under a shared `EdgeFade` gradient strip, so content passes faintly behind the status bar and home indicator; iOS Reduce Transparency falls back to the previous solid band.

## Dev environment

Built with Claude Code. Matt Pocock's workflow skills (`/tdd`, `/to-spec`, `/to-tickets`, `/triage`, `/domain-modeling`, …) are **vendored into this repo and committed**, not installed globally: they ship as the `mattpocock-skills` plugin, committed at [`.agents/plugins/mattpocock-skills/`](.agents/plugins/mattpocock-skills) and loaded from a relative path, so the pinned content travels with the repo instead of living in a per-machine install. Marketplace state is per-user, so a new machine needs the folder trusted plus one throwaway session before the skills appear. Canonical skill names are namespaced (`/mattpocock-skills:tdd`) with the **bare** form (`/tdd`) resolving as an alias; the one exception is `code-review`, where the bare name belongs to Claude Code's built-in and Matt's is `/mattpocock-skills:code-review`. The pinned commit and the update procedure live in [`.agents/plugins/README.md`](.agents/plugins/README.md). Argent's skills use the older hand-vendored mechanism (`.agents/skills/` + `.claude/skills/` symlinks + `skills-lock.json`). `.claude/settings.json` declares both, plus the Expo plugin. Codex reaches the same pinned set through relative symlinks in [`.agents/skills/`](.agents/skills); `pnpm agents:check` asserts both legs. See [`AGENTS.md`](AGENTS.md) for the full working agreement.
