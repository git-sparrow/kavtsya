...
- Matt Pocock's workflow skills (tdd, to-spec, to-tickets, triage, domain-modeling, …) ship as the `mattpocock-skills` plugin, **vendored into this repo and committed** at `.agents/plugins/mattpocock-skills/` and loaded from a relative path, so the pinned content is in version control rather than in someone's `~/.claude`. Marketplace state is per-user, so a new machine needs the workspace trusted and one throwaway session before the skills appear (they register on the first run, load from the next) — that, the pinned commit, the update procedure, and the CI caveat are in `.agents/plugins/README.md`. Update deliberately, in its own PR, the way we treat argent; a local marketplace does not auto-update, which is the point. Canonical skill names are namespaced (`/mattpocock-skills:tdd`) and the bare form (`/tdd`) resolves as an alias — except `code-review`, where the bare name is Claude Code's built-in and Matt's is `/mattpocock-skills:code-review`. Argent's skills use a different mechanism — hand-vendored files in `.agents/skills/` with `.claude/skills/` symlinks and `skills-lock.json` hashes, emitted by `argent init --local`. `.claude/settings.json` declares both, plus the Expo plugin.
